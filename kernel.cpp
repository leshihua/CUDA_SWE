...
Shallow2* qinit(int m, int caseNo) //should include ghost cells
{
    ...
    else if (2 == caseNo)//1-shocek, 2-rarefaction
...
{
    //todo: maybe I can update q inside this rp1 function
    //Shallow2* wave1 = (Shallow2*) malloc((mx+1)*sizeof(Shallow2));//1-wave
    //Shallow2* wave2 = (Shallow2*) malloc((mx+1)*sizeof(Shallow2));//2-wave
    //Shallow2 amdq = (Shallow2*) malloc((mx+1)*sizeof(Shallow2)); //A^- \delta Q defined at cell edge
    //Shallow2 apdq = (Shallow2*) malloc((mx+1)*sizeof(Shallow2)); //A^+ \delta Q defined at cell edge
    //float* s1 = (float*) malloc((mx+1)*sizeof(float));//speed of 1-wave
    //float* s2 = (float*) malloc((mx+1)*sizeof(float));//speed of 2-wave
    ...
    //todo: print CFL number
        for (int i = 0; i < (MX+1); i++) 
        {
            ...
            else if (s1[i] < 0.f)//1-wave is left-going
            {
                ...
                float hl2 = q[i+1].h - wave2[i+1].h;
                float uhl2 = q[i+1].hu - wave2[i+1].hu;
...
void godunov_serial(Shallow2 *q, const float* const x, const float dt, const int nsteps, const float dx) 
{
    ...
    for (int n = 0; n < nsteps; n ++)
    {
        ...
        apply_BC(q);//apply B.C.
        rp1(q, amdq, apdq, wave1, wave2, s1, s2, dt, dx, EFIX); // q should include ghost cells
        if ((n+1) % OUTPUT_FREQUENCY == 0 )//write output
        {
            std::cout<<"Write data at t = "<<std::to_string(t)<<" to file."<<std::endl;
            std::ofstream outfile;
            outfile.open("output_"+std::to_string(t));
            for (int j = MBC; j < MX+MBC; j++)
            {
                outfile<<std::to_string(x[j])<<","<<std::to_string(q[j].h)<<","<<std::to_string(q[j].hu)<<std::endl;
            }
...
float* generateMesh(float x_lower, float x_upper)
{
    ...
    for (int i = MBC; i < (MX+MBC); i++)
    {
        x[i] = x_lower + (i-MBC)*dx;
    }
    for (int i = 0; i < MBC; i++)
    {
        x[i] = x_lower - (MBC-i)*dx;
        x[MX+MBC+i] = x_upper + (i+1)*dx;
    }
    return x;
}
